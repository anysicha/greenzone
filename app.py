...
사용자 흐름:
1) 자연어로 조경 조건 입력
2) Gemini API가 조건을 이해해 내부 변수로 변환
3) Hard Constraint Filter로 생존 불가능 수종 제거
4) PuLP 기반 최적화 Solver로 탄소흡수량 최대 식재 포트폴리오 산출
5) 사용자 친화형 결과 화면/차트/리포트 제공
"""
...
with st.sidebar:
    ...
    st.divider()c
    st.caption("이 영역은 시연 운영자용입니다. 실제 배포 시에는 숨길 수 있습니다.")

st.markdown(
    """
    <div class="hero">
        <h1>🌿 GreenCity AI</h1>
        <p>자유롭게 녹화 공간을 설명하면, AI가 조건을 이해하고 최적의 식재 포트폴리오를 추천해드립니다.</p>
    </div>
    """,
    unsafe_allow_html=True,
)

st.markdown("### 어떤 공간을 녹화하고 싶나요?")
st.caption("예산, 면적, 공간 유형을 문장으로 편하게 입력해보세요.")
...
selected_example = st.selectbox("빠른 예시", ["직접 입력"] + list(example_prompts.keys()))
...
run_btn = st.button("🌱 맞춤 녹화 설계 받기", type="primary", use_container_width=True)
...
maint_label = "제한 없음"
if parsed.get("maint_level_max") == 2:
    maint_label = "쉬운 관리"
elif parsed.get("maint_level_max") == 3:
    maint_label = "보통 관리"
elif parsed.get("maint_level_max") == 5:
    maint_label = "전문 관리 가능"
cond4.metric("관리 수준", maint_label)
